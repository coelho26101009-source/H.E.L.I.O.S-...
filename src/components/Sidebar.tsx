...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-[#13131f] border-r border-amber-500/10 z-50 flex flex-col transform transition-transform duration-300 ease-out
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-white/5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold tracking-widest text-amber-500 text-sm">H.E.L.I.O.S.</span>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-white/30 hover:text-white/70 hover:bg-white/6 transition-all lg:hidden"
          >
            <X size={15} />
          </button>
        </div>
            ...
            <div className="flex items-center gap-3 px-2 py-2 rounded-xl bg-amber-500/5 border border-amber-500/10">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="text-sm font-semibold text-amber-400 truncate">
                  {user.displayName?.split(' ')[0] || 'Utilizador'}
                </span>
        ...
        <div className="px-3 pt-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-amber-500/8 hover:bg-amber-500/14 border border-amber-500/18 hover:border-amber-500/35 text-amber-400 text-sm font-semibold transition-all duration-150"
          >
            <Plus size={15} />
            Nova conversa
          </button>
          ...
          {isGuest && (
            <div className="px-2 py-4 text-center">
              <p className="text-xs text-white/25 mb-3 leading-relaxed">
                Entra com a tua conta para guardar o histório de conversas.
              </p>
              <button
                onClick={onLogin}
                className="flex items-center justify-center gap-2 w-full py-2 px-3 rounded-lg bg-amber-500/8 border border-amber-500/15 text-amber-500/70 hover:text-amber-400 text-xs font-semibold transition-all"
              >
                <LogIn size={13} />
                Entrar com Google
              </button>
        ...
        {user && (
          <div className="px-3 pb-4 border-t border-white/5 pt-3">
            <button
              onClick={onLogout}
              className="w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-red-500/50 hover:text-red-400 hover:bg-red-500/6 border border-transparent hover:border-red-500/15 text-sm font-medium transition-all"
            >
              <LogOut size={14} />
              Terminar sessão
            </button>
