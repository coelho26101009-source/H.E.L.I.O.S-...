        ...
        <div className="flex items-end gap-2 px-4 py-3 rounded-2xl bg-[#1a1a28] border border-amber-500/15 focus-within:border-amber-500/35 focus-within:shadow-[0_0_0_3px_rgba(245,158,11,0.05)] transition-all duration-200">
          <textarea
            ...
            placeholder={
              ...
              isLoading ? 'H.E.L.I.O.S. está a pensar...' :
              'Pergunta ao H.E.L.I.O.S. algo...'
            }
            className="flex-1 bg-transparent border-none outline-none resize-none text-[#ececec] text-sm placeholder:text-white/20 leading-relaxed min-h-[22px] max-h-[140px] scrollbar-none disabled:opacity-50 font-['Sora']"
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              className="w-8 h-8 rounded-lg flex items-center justify-center text-white/25 hover:text-white/60 hover:bg-white/6 transition-all"
            >
              <Paperclip size={16} />
            </button>
            ...
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isMuted ? 'text-red-400 bg-red-500/10' : 'text-white/25 hover:text-white/60 hover:bg-white/6'}`}
            >
            ...
            <button
              onClick={onToggleMic}
              title={isListening ? 'Parar microfone' : 'Microfone'}
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isListening ? 'text-red-400 bg-red-500/10 animate-pulse' : 'text-white/25 hover:text-white/60 hover:bg-white/6'}`}
            >
            ...
            <button
              ...
              disabled={!canSend}
              title="Enviar"
              className="w-8 h-8 rounded-lg flex items-center justify-center bg-amber-500 hover:bg-amber-400 text-black transition-all disabled:opacity-25 disabled:cursor-not-allowed disabled:hover:bg-amber-500 shadow-[0_2px_10px_rgba(245,158,11,0.25)] hover:shadow-[0_4px_16px_rgba(245,158,11,0.35)] hover:-translate-y-px active:translate-y-0"
            >
              <Send size={15} />
            </button>
          </div>
